...
//  ----------------------------------------------------------------------------
     ...
     */
    CLI.getStore = function () {
        // we cache the store here just in case we need to
        // use it multiple times during a command execution.
        if (!CLI._store) {
            CLI._store = Store.createStore({
                root: process.cwd(),
                skipPreload: true
            });
            CLI._store.preloadResourceVersions();
        }
        return CLI._store;
    };

    /*
     * Lookup for command thru the store.
     * @param {string} commandName The name of the command to execute.
     * @return {Object} The command object if exists.
     */
    CLI.getCommand = function (commandName) {
        var command,
            store,
            allMatchingCommands,
            i;
        ...
        store = CLI.getStore();
        allMatchingCommands = store.getResourceVersions({
            type: 'command',
            name: commandName
        });
        if (allMatchingCommands && allMatchingCommands[0] &&
                allMatchingCommands[0].source &&
                allMatchingCommands[0].source.fs) {
            command = require(allMatchingCommands[0].source.fs.fullPath);
        } else {
    ...
    CLI.run = function() {
        ...
        try {
            ...
            utils.error('Error loading command: ' + commandName + ' ' + e.message,
        ...
        // adding a reference to CLI into the command object,
        // this trick allows up to use getCommand and getStore
        // from any command without creating a new instance of it.
        // TODO: there might be a better way! probably passing it
        //       as a reference in the "run" call.
        command.CLI = CLI;
