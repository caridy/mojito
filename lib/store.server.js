...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        /**
         * Returns the content and the stat object for a resource.
         * @method getResourceContent
         * @param {object} res the resource object
         * @param {function(err,data,stat)} callback callback used to return the resource content (or error)
         * @return {string} the buffer that represents the resource
         * @return {string} the buffer that represents the resource
         */
        getResourceContent: function(res, callback) {
            ...
            if (res && res.source && res.source.fs) {
                ...
                fs.stat(filename, function(err, stat) {
                    ...
                    if (err) {
                        Y.log('err finding: ' + filename, 'warn', NAME);
        ...
        /**
         * Returns the compiled content for a resource if there is
         * a compiled defined as part of the res, otherwise returns
         * the raw version of it.
         * @method getReadCompiled
         * @param {object} res the resource object
         * @param {function(err,data,stat)} callback callback used to return the resource content (or error)
         * @return {string} the buffer that represents the resource
         */
        getResourceCompiled: function(res, callback) {
            if (res && res.source && res.source.fs) {

                if (res.compile) {
                    // the resource store has its own get mechanism
                    // to pre-processing produce a buffer
                    res.compile(callback);
                } else {
                    this.getResourceContent(res, callback);
                }
